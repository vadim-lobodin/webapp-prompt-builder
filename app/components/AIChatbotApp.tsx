                        ...
                        try {
                                const response = await openai.chat.completions.create({
                                        ...
                                        messages: [
                                                {
                                                        role: "system",
                                                        content: "You are an AI assistant creating a concise combined prompt for building an app based on the user's choices. Summarize the key features and requirements into a brief, clear prompt for app development. Include summary, key features, requirements and additional details. Keep it under 150 words. Don't use markdown"
                                                },
                                                ...messages.map(msg => ({
                                                        role: msg.isUser ? "user" : "assistant",
                                                        content: msg.content
                                                })) as OpenAI.Chat.ChatCompletionMessageParam[],
                                                { role: "user", content: "Based on our conversation, create a concise combined prompt for building this app." }
                                        ],
                                });

                                const combinedPromptContent = response.choices[0].message.content;
                                if (combinedPromptContent) {
                                        setCombinedPrompt(combinedPromptContent);
                                } else {
                                        throw new Error("No content in AI response for combined prompt");
                                }
                        } catch (error) {
                                console.error('Error generating combined prompt:', error);
                                addMessage("I'm sorry, I encountered an error generating the combined prompt. Please try again.", false);
                        } finally {
                                ...
                                <>
                                        ...
                                        {stage === 'completed' && (
                                            <div className="mt-8 p-6 bg-white rounded-lg shadow-lg">
                                                    <h3 className="text-2xl font-semibold mb-4">Final Prompt for Your App</h3>
                                                    {isGeneratingFinalPrompt ? (
                                                        <div className="flex flex-col items-center justify-center space-y-4">
                                                                <div className="w-12 h-12 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
                                                                <p className="text-lg font-medium text-gray-700">Generating final prompt...</p>
                                                        </div>
                                                    ) : combinedPrompt ? (
                                                        ...
                                                        <p className="text-red-500">Failed to generate the final prompt. Please try again.</p>
                                                    )}
                                            </div>
                                        )}
                                </>
